...
- **Security Headers** (`core/SecurityHeaders.php`): zentrale Browser-Hardening-Header fuer alle Core-Responses — CSP mit Same-Origin-Policy fuer Skripte/Styles/Fonts/Fetches, `frame-ancestors 'self'`, `X-Frame-Options: SAMEORIGIN`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin` und restriktive `Permissions-Policy`. `script-src` erlaubt keine Inline-Skripte mehr; `style-src` erlaubt Inline-Styles vorerst weiter, weil Admin-Oberflaeche, Editor und Themes noch Style-Attribute nutzen.
