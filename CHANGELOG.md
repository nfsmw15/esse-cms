...
- **Theme-gerendertes Login/Passwort-zurücksetzen im esse-base-Theme**: `/login`, `/admin/forgot-password` und `/admin/reset-password` werden jetzt über die Hooks `auth.login.render`, `auth.forgot_password.render` und `auth.reset_password.render` (neue Partials `themes/esse-base/templates/{login,forgot-password,reset-password}.php`) im normalen Seiten-Layout mit Navbar und Footer dargestellt — wie bereits bei `/registrieren` — statt im separaten Admin-Look. `/admin/login` bleibt unverändert als Fail-Safe.
- **Einheitliches Formular-Layout für `/registrieren`**: Das Registrierungsformular (`pages/registrieren.php`) wird jetzt wie `/login` und `/admin/forgot-password` in einer Karte dargestellt; die doppelte Überschrift (Seitentitel im Layout + eigenes `<h1>`) wurde entfernt.
