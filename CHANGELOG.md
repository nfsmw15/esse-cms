...
- **Security Headers** (`core/SecurityHeaders.php`): zentrale Browser-Hardening-Header fuer alle Core-Responses — CSP mit Same-Origin-Policy fuer Skripte/Styles/Fonts/Fetches, `frame-ancestors 'self'`, `X-Frame-Options: SAMEORIGIN`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin` und restriktive `Permissions-Policy`. Die CSP erlaubt vorerst Inline-Skripte/-Styles, weil Admin-Oberflaeche und Themes aktuell noch inline arbeiten, blockt aber externe Fremdquellen und gefaehrliche Einbettungen.
...
- **CSP-Vorbereitung / Inline-JS-Reduktion**: Passkey-Login, Profil-Passkey-Registrierung, Rollen-Rechte-Toggles, Updater-SSE, Menue-Erstellung, Icon-Picker, esse-ui Tabs/Alerts und mehrere Admin-Bestaetigungsdialoge wurden von Inline-Skripten/Event-Attributen auf externe Assets unter `public/assets/js/` und JSON-Konfig-Bloecke umgestellt. Die CSP erlaubt `unsafe-inline` vorerst noch, bis die verbleibenden groesseren Admin-Flows (Seitenliste und Menueeditor) ebenfalls migriert sind.
