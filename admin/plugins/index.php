...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'install_from_repo') {
        ...
        if ($fullName && preg_match('#^[a-zA-Z0-9\-]+/[a-zA-Z0-9\-\.]+$#', $fullName)) {
            // manage_plugins erlaubt die Installation, ersetzt aber nicht die Kanalverwaltung
            // (manage_repos) — ohne diese Pruefung koennte ein Nutzer mit nur manage_plugins
            // jedes beliebige GitHub-Repo installieren, auch ausserhalb der konfigurierten Kanaele.
            if (!packageRepoChannelAllowed($fullName)) {
                AuditLog::record('plugin_install_failed', Auth::id(), Auth::user()['email'] ?? null, ['source' => 'repo', 'repo' => $fullName, 'reason' => 'channel_not_allowed']);
