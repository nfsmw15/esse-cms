...
class AuthenticatorData {
    ...
    private function _readAttestData($binary, &$endOffset) {
        ...
        // W3C WebAuthn Level 3 §5.8.3: credential ID length MUST be in [16, 1023]
        if ($length < 16 || $length > 1023) {
            throw new WebAuthnException('Invalid credential ID length', WebAuthnException::INVALID_DATA);
        }

        // Guard against declared length exceeding the available binary
        if (55 + $length > \strlen($binary)) {
            throw new WebAuthnException('Credential ID length exceeds attested data', WebAuthnException::INVALID_DATA);
        }
